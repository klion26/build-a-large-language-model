...
/// Listing 3.5
/// An efficient implementation of Multi_head Attention
...
impl Module for MultiHeadAttention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let (b, num_tokens, _d_in) = xs.dims3()?;
        println!(
            "xs dims:{:?}, b:{:?}, num_tokens:{:?}, _d_in:{:?}",
            xs.dims(),
            b,
            num_tokens,
            _d_in
        );
        println!("self.key dims:{:?}", self.w_key.weight().dims());
        let queries = self.w_query.forward(xs)?;
        let keys = self.w_key.forward(xs)?;
        let values = self.w_value.forward(xs)?;
        ...
        let queries = queries
            ...
            .transpose(1, 2)?
            .contiguous()?;
        println!("keys dims:{:?}", keys.dims());
        let keys = keys
            .reshape((b, num_tokens, self.num_heads, self.head_dim))?
            .transpose(1, 2)?
            .contiguous()?;
        println!("keys dims after transpose:{:?}", keys.dims());
        let values = values
            .reshape((b, num_tokens, self.num_heads, self.head_dim))?
            .transpose(1, 2)?
            .contiguous()?;
