...
impl Exercise for X4P2 {
    ...
    fn main(&self) {
        ...
        for (mdl_name, cfg) in config.iter() {
            ...
            println!("GPT modle :{} total params: {}", mdl_name, total_params);
...
impl Exercise for X4P3 {
    ...
    fn main(&self) {
        ...
        let model = GPTModel::new_v2(addons::ConfigV2::get_config_124m(), vb).unwrap();
...
mod addons {
    ...
    impl TransformerBlock {
        fn new_v2(cfg: ConfigV2, vb: VarBuilder<'_>) -> Result<Self> {
            ...
            let ln1 = LayerNorm::new(cfg.emb_dim, vb.pp("norm1"))?;
            let ln2 = LayerNorm::new(cfg.emb_dim, vb.pp("norm2"))?;
            let drop_shortcut = Dropout::new(cfg.drop_rate_shortcut);
            TransformerBlock::from_fields(att, ff, ln1, ln2, drop_shortcut)
        }
    }
    ...
    impl GPTModel {
        pub fn new_v2(cfg: ConfigV2, vb: VarBuilder<'_>) -> Result<Self> {
            ...
            for i in 0..cfg.n_layers {
                trf_blocks =
                    trf_blocks.add(TransformerBlock::new_v2(cfg, vb.pp(format!("trf-{}", i)))?);
            }
