...
impl Example for EG01 {
    ...
    fn main(&self) {
        use crate::listings::{
            ...
            ch05::{text_token_ids, token_ids_to_text},
        };
        ...
        let token_ids = generate_text_simple(
            model,
            text_token_ids(start_context, &tokenizer, vb.device()).unwrap(),
            max_new_tokens,
            cfg.context_length,
        )
        .unwrap();
...
impl Example for EG02 {
    fn description(&self) -> String {
        String::from("Example computation of cross-entropy and perplexity.")
    }
    ...
    fn main(&self) {
        ...
        // lets see the predicted probas for the target tokens
        let text_ids = 0_usize;
        let target_probas_1 =
            addons::get_target_token_probas_helper(text_ids, &targets, &probas, vb.device())
                .unwrap();

        println!("Text 1: {:?}", target_probas_1);
        let text_ids = 1_usize;
        let target_probas_2 =
            addons::get_target_token_probas_helper(text_ids, &targets, &probas, vb.device())
                .unwrap();
...
mod addons {
    use crate::listings::ch02::{create_dataloader_v1, GPTDatasetIter, GPTDatasetV1};
    ...
    pub fn get_target_token_probas_helper(
        ...
        let mut target_probase_1: Vec<f32> = vec![];
        for (i, target_token) in target_tokens_1.iter().enumerate() {
            let target_proba = probas
                .i((text_idx, i, *target_token as usize))
                .unwrap()
                .to_scalar::<f32>()?;
            target_probase_1.push(target_proba);
        }
        Tensor::from_vec(target_probase_1, target_tokens_1.len(), dev)
    }
    ...
    pub fn get_train_val_data_loaders(
        ...
        let (_train_dataset, train_loader) =
            create_dataloader_v1(train_data, batch_size, max_length, stride, true, true);
        let (_val_dataset, val_loader) =
            create_dataloader_v1(val_data, batch_size, max_length, stride, true, true);

        (_train_dataset, train_loader, _val_dataset, val_loader)
    }
}
